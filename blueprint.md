...
*   **Window Management:** A flexible windowing system allows you to open, close, and manage multiple applications in a clean, organized workspace. Windows can be minimized, fullscreened, and dragged. A UI info bit slides down saying `[esc] to exit fullscreen`, and disappears after 5 seconds.
...
*   **Game Detail View:** Clicking on a game in either view opens a detail view with more information and a "Play" button.
*   **Game Player:** When the "Play" button is clicked, the game loads in an iframe. The window title changes to reflect the game being played, and the top bar (search and view switcher) of the Cyanide app is hidden. Pressing `Esc` exits the game.
